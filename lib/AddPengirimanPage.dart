...
class _PengirimanListPageState extends State<PengirimanListPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => _navigateToAddPengiriman(),
        icon: Icon(Icons.add),
        label: Text('Tambah'),
  ...
  void _navigateToAddPengiriman() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => AddPengirimanPage()),
    );
  }
...
void _printInvoice(QueryDocumentSnapshot doc) async {
  ...
  // Add the page with the QR code and other information
  pdf.addPage(
    pw.MultiPage(
        ...
        return [
          pw.Column(
            ...
            children: [
              ...
              // Print each barangKiriman item with a signature table next to it
...
class HistoryPengirimanPage extends StatefulWidget {
  @override
  _HistoryPengirimanPageState createState() => _HistoryPengirimanPageState();
}
...
class _HistoryPengirimanPageState extends State<HistoryPengirimanPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: _deleteOldRecords,
        child: Icon(Icons.delete),
        tooltip: 'Hapus data lebih dari 2 bulan',
  ...
  void _deleteOldRecords() async {
    DateTime twoMonthsAgo = DateTime.now().subtract(Duration(days: 60));
    QuerySnapshot snapshot = await FirebaseFirestore.instance
        .collection('historyPengiriman')
        .where('tanggal', isLessThan: Timestamp.fromDate(twoMonthsAgo))
        .get();
    ...
    if (snapshot.docs.isNotEmpty) {
      ...
      if (confirmDelete) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Data yang berumur lebih dari 2 bulan telah dihapus.')));
      }
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Tidak ada data yang berumur lebih dari 2 bulan.')));
    }
  }
}
...
class _BarangListState extends State<BarangList> {
  ...
  String _sortBy = 'NamaPenerima'; // Default sorting by 'NamaPenerima'
      ...
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton<String>(
            onSelected: (value) {
              setState(() {
                _sortBy = value.split('_')[0];
                _isAscending = value.split('_')[1] == 'asc';
              });
            },
            itemBuilder: (context) => [
              PopupMenuItem(
                value: 'NamaPenerima_asc',
                child: Text('Sort by Nama Penerima (A-Z)'),
              ),
              PopupMenuItem(
                value: 'NamaPenerima_desc',
                child: Text('Sort by Nama Penerima (Z-A)'),
              ),
              /*
              PopupMenuItem(
                value: 'createdAt_desc',
                child: Text('Sort by Time (Newest First)'),
              ),
              PopupMenuItem(
                value: 'createdAt_asc',
                child: Text('Sort by Time (Oldest First)'),
              ),
          ...
          Expanded(
            child: StreamBuilder(
              ...
              builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
                ...
                // Sorting the filtered documents
                filteredDocs.sort((a, b) {
                  if (_sortBy == 'NamaPenerima') {
